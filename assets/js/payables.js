(function () {
  ...
  function openPayableDelete(id) {
    ...
    const testCleanupBlockers = testCleanupBlocked ? testCleanupPreview.blockers.map((row) => `<li><strong>${esc(row.label)}</strong><span>${esc(row.message)}</span>${row.count !== undefined ? `<small>數量：${esc(row.count)}</small>` : ''}</li>`).join('') : '';
