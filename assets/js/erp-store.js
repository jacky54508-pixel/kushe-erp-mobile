(function () {
  ...
  const FINAL_HISTORICAL_CLEANUP_TARGETS=Object.freeze({
    fuhua:{receivableId:'ms7l97t7d1m1b7',sourceNo:'B458413',projectId:'ms7l8l5pl9jel7',projectName:'富華-心之所向',customerName:'富華創新',date:'2026-07-29',groupId:'ms7l8l5pbt6cz1',dailyLogIds:['msethpigr5vn88','msethpigaddmde'],employeeNames:['劉佳勳','柯智耀'],billingId:'legacy-billing-ms7l97t7d1m1b7',itemName:'電梯內扇',unit:'面',qty:39,price:1000,untaxed:39000,tax:1950,gross:40950},
  ...
  function finalHistoricalCleanupPlan() {
    ...
    const dailyChecks=dailyRows.filter((entry)=>entry.row).map(({row})=>{const items=row.items||[],item=items[0]||{},subtotal=num(financialAuditFirst(item,['untaxedSubtotal','preTaxAmount','amount'],num(item.qty??item.quantity)*num(item.price??item.unitPrice))),facts={id:financialAuditText(row.id),groupId:financialAuditText(row.groupId||row.id),date:financialPhase2Date(row),employeeName:financialAuditText(row.employeeName||(state.employees||[]).find((employee)=>financialAuditText(employee.id)===financialAuditText(row.employee))?.name),projectId:financialAuditText(row.project||row.projectId),projectName:financialPhase2Project(row).name,performance:num(row.performance),itemCount:items.length,item:{name:financialAuditText(item.item||item.itemName),unit:financialAuditText(item.unit),qty:num(item.qty??item.quantity),price:num(item.price??item.unitPrice),subtotal,workItemId:financialAuditText(item.workItemId),billingId:financialAuditText(item.billingId),billingNo:financialAuditText(item.billingNo),billingStatus:financialAuditText(item.billingStatus)},billingId:financialAuditText(row.billingId),billingNo:financialAuditText(row.billingNo),billingStatus:financialAuditText(row.billingStatus)};const valid=facts.groupId===f.groupId&&facts.date===f.date&&facts.projectId===f.projectId&&sameName(facts.projectName,f.projectName)&&f.employeeNames.includes(facts.employeeName)&&financialAuditMoneyEqual(facts.performance,f.untaxed)&&facts.itemCount===1&&facts.item.name===f.itemName&&facts.item.unit===f.unit&&financialAuditMoneyEqual(facts.item.qty,f.qty)&&financialAuditMoneyEqual(facts.item.price,f.price)&&financialAuditMoneyEqual(facts.item.subtotal,f.untaxed)&&Boolean(facts.item.workItemId)&&!facts.billingId&&!facts.billingNo&&facts.billingStatus!=='已請款'&&!facts.item.billingId&&!facts.item.billingNo&&facts.item.billingStatus!=='已請款';return {valid,facts,fingerprint:financialRepairFingerprint(row)}});
    if(dailyChecks.length!==2||dailyChecks.some((row)=>!row.valid)||new Set(dailyChecks.map((row)=>row.facts.employeeName)).size!==2)block('FUHUA_DAILY_FACTS_CHANGED','富華兩筆 Daily 必須為同 group、不同員工、相同唯一施工內容且仍未請款。',{dailyChecks});
    ...
    const fuhua={classification:'SAME_WORK_GROUP_TWO_EMPLOYEES',receivable:fuhuaFacts,receiptAudit:{historyCount:fuhuaReceiptAudit.length,latestAction:latestReceiptAudit?.action||'',receivedMustRemain:0},sourceRef,sourceMatchProof:{matchCount:sourceMatchesRows.length,dailyLogIds:sourceMatchIds,sameGroup:true,sameItemIndex:true,constructionCopies:2,billingLineCount:1,billingConstructionAmount:f.untaxed},proposedBilling,receivablePatch,dailyPatches,protected:{receivableFingerprint:financialRepairFingerprint(receivable),dailyFingerprints:dailyChecks.map((row)=>({id:row.facts.id,fingerprint:row.fingerprint}))}};
    ...
    const w=targets.weiyuan,weiyuanEntries=w.paymentIds.map((id)=>finalHistoricalCleanupOne(state.payments,id,'威沅 legacy Payment')),weiyuanRows=weiyuanEntries.map((entry)=>entry.row).filter(Boolean),weiyuanFacts=weiyuanRows.map((row)=>finalHistoricalCleanupPaymentFacts(row,w));
    ...
    const canonicalPayment=weiyuanRows.find((row)=>financialAuditText(row.id)===canonicalSelection.canonicalId),proposedPayable={id:w.payableId,payableNo:w.payableNo,date:w.date,vendor:canonicalPayment?.vendor||canonicalPayment?.vendorId||'',vendorName:w.vendorName,amount:w.amount,paid:w.amount,status:'已付清',sourceType:'legacy-confirmed-payable',category:'材料採購',note:'歷史真實應付／付款，由使用者確認；原 legacy summary 無銀行流水，不得虛構銀行資料。'},canonicalPaymentPatch={id:canonicalSelection.canonicalId,patch:{payableId:w.payableId},protectedFingerprint:financialRepairFingerprint(finalHistoricalCleanupOmit(canonicalPayment,['payableId']))},duplicatePaymentDelete={id:canonicalSelection.duplicateId,classification:'DELETE_DUPLICATE_LEGACY_PAYMENT_CANDIDATE',fingerprint:financialRepairFingerprint(weiyuanRows.find((row)=>financialAuditText(row.id)===canonicalSelection.duplicateId))};
    const weiyuan={classification:canonicalSelection.canonicalId?'REBUILD_CONFIRMED_LEGACY_PAYABLE':'BLOCK',truth:'USER_CONFIRMED_REAL_PAYABLE_PAYMENT',canonicalPaymentId:canonicalSelection.canonicalId,duplicatePaymentId:canonicalSelection.duplicateId,selectionEvidence:canonicalSelection.selectionEvidence,proposedPayable,canonicalPaymentPatch,duplicatePaymentDelete,bankTransactionCreates:[],bankTransactionPatches:[],facts:weiyuanFacts};
