(function () {
  ...
  function openDailyDrawer(batchId='') {
    ...
    projectGroups.forEach((log)=>{(log.items||[]).forEach((item)=>lines.push({
      project:log.project,house:item.house||'',item:item.itemName||item.item||'',unit:item.unit||'式',qty:number(item.qty),inputPrice:number(item.unitPrice??item.inputPrice??item.price),taxMode:item.taxMode||'未稅',billable:item.billable!==false,workItemId:item.workItemId||'',sourceType:item.sourceType||(item.quotationId?'quotation':'manual'),quotationId:item.quotationId||item.quoteId||'',quotationLineId:item.quotationLineId||item.quoteLineId||'',quotationNo:item.quotationNo||'',pricingType:item.pricingType||'actual',lumpSumAmount:number(item.lumpSumAmount)
    }))});
    if(!lines.length)lines=[{project:'',house:'',item:'',unit:'式',qty:1,inputPrice:0,taxMode:'未稅',billable:true,workItemId:'',sourceType:'manual',pricingType:'actual'}];
    const workLog=logs.find((log)=>log.workMode&&log.workMode!=='none')||{},commissionEnabled=!logs.length||logs.some((log)=>number(log.performance)>0),layer=$('#commissionDrawerLayer');
    ...
    const rowDraft=(row,house=$('.daily-line-house',row).value.trim(),workItemId=row.dataset.workItemId||'')=>({project:$('.daily-line-project',row).value,house,item:row.dataset.itemName||$('.daily-line-item',row).value.trim(),itemName:row.dataset.itemName||$('.daily-line-item',row).value.trim(),unit:$('.daily-line-unit',row).value.trim()||'式',qty:number($('.daily-line-qty',row).value),inputPrice:number($('.daily-line-price',row).value),unitPrice:number($('.daily-line-price',row).value),taxMode:$('.daily-line-tax',row).value,billable:row.dataset.pricingType!=='lump_sum'&&$('.daily-billable input',row).checked,sourceType:row.dataset.sourceType||'manual',pricingType:row.dataset.pricingType||'actual',quotationId:row.dataset.quotationId||'',quotationLineId:row.dataset.quotationLineId||'',quotationNo:row.dataset.quotationNo||'',lumpSumAmount:number(row.dataset.lumpSumAmount),workItemId});
    const groupKey=(project,house)=>JSON.stringify([String(project||''),String(house||'').trim()]);
    const groupRows=(group)=>$$('.daily-line',group);
    const groupProject=(group)=>$('.daily-house-project',group).value;
    const groupHouse=(group)=>$('.daily-house-name',group).value.trim();
    ...
    const emptyLine=(project='',house='')=>({project,house,item:'',unit:'式',qty:1,inputPrice:0,taxMode:'未稅',billable:true,workItemId:'',sourceType:'manual',pricingType:'actual'});
    const appendLine=(group,line,order)=>{const tbody=$('tbody',group);tbody.insertAdjacentHTML('beforeend',lineHtml(line));const row=tbody.lastElementChild;row.dataset.draftOrder=String(order??draftSequence++);bindRow(row);return row};
    const closeBatch=()=>{batchLayer.hidden=true;batchTemplate=null;batchInput.value=''};
    const openBatch=(group)=>{if(!groupProject(group)){window.KushePhase1?.toast('請先選擇案場');$('.daily-house-project',group).focus();return}batchTemplate=group;batchInput.value='';batchLayer.hidden=false;requestAnimationFrame(()=>batchInput.focus())};
    const bindRow=(row)=>{
      const project=$('.daily-line-project',row),item=$('.daily-line-item',row);
      project.onchange=()=>{item.value='';clearQuote(row);$('.daily-line-unit',row).value='式';$('.daily-line-price',row).value='0';$('.daily-line-tax',row).value='未稅';$('.daily-billable input',row).checked=true;refreshChoices(row);calc()};
      let composing=false;
      const choose=(event)=>{if(composing||event?.isComposing)return;applyChoice(row);calc()};
      item.oncompositionstart=()=>{composing=true};
      item.oncompositionend=()=>{composing=false;choose()};
      item.oninput=choose;item.onchange=choose;
      $$('input,select',row).forEach((input)=>{if(input!==project&&input!==item){input.oninput=calc;input.onchange=calc}});
      $('.daily-line-clone',row).onclick=()=>{const draft=cloneDailyLineDraft(rowDraft(row)),existing=findGroup(draft.project,''),group=existing||createGroup(draft.project,'',[]);appendLine(group,draft);calc();$('.daily-house-name',group).focus()};
      $('.daily-line-remove',row).onclick=()=>{const group=row.closest('.daily-house-group');if(groupRows(group).length>1)row.remove();else{const order=number(row.dataset.draftOrder);row.remove();appendLine(group,emptyLine(groupProject(group),groupHouse(group)),order)}calc()};
      // Hydrate saved/clone values without re-selecting a quote or changing its snapshot.
      updateHint(row);
    };
    const createGroup=(project,house,groupLines)=>{
      ...
      group.innerHTML='<header class="daily-house-header"><div class="daily-house-project-control"><label><span>客戶／案場</span><select class="daily-house-project">'+projectOptions(project)+'</select></label><button type="button" class="commission-secondary daily-quick-project-open">＋ 新增案場</button></div><label><span>戶別</span><input class="daily-house-name" value="'+esc(house||'')+'" placeholder="未指定戶別"></label><div class="daily-house-amount"><span>戶別施工小計</span><b class="daily-house-subtotal">$0</b></div><div class="daily-house-actions"><button type="button" class="commission-secondary daily-house-batch">批次複製整戶</button><button type="button" class="commission-secondary daily-house-remove">刪除整戶</button></div></header><div class="daily-lines-wrap"><table class="daily-lines-table daily-house-items"><colgroup><col class="daily-col-item"><col class="daily-col-unit"><col class="daily-col-qty"><col class="daily-col-tax"><col class="daily-col-price"><col class="daily-col-total"><col class="daily-col-purpose"><col class="daily-col-actions"></colgroup><thead><tr><th>施工項目</th><th>單位</th><th>數量</th><th>稅別</th><th>單價</th><th>小計</th><th>用途</th><th>操作</th></tr></thead><tbody></tbody></table></div><footer><button type="button" class="commission-secondary daily-house-add">＋ 新增施工項目</button></footer>';
      body.appendChild(group);
      groupLines.forEach((line)=>appendLine(group,line,line.draftOrder));
      const projectInput=$('.daily-house-project',group),houseInput=$('.daily-house-name',group);
      const syncHouse=()=>{groupRows(group).forEach((row)=>{$('.daily-line-house',row).value=houseInput.value.trim()})};
      const acceptHeader=()=>{const nextProject=projectInput.value,nextHouse=houseInput.value.trim();if(nextProject&&findGroup(nextProject,nextHouse,group)){projectInput.value=group.dataset.project;houseInput.value=group.dataset.house;syncHouse();window.KushePhase1?.toast('此案場戶別已存在，未合併或覆蓋');return false}const changedProject=nextProject!==group.dataset.project;group.dataset.project=nextProject;group.dataset.house=nextHouse;syncHouse();if(changedProject)groupRows(group).forEach((row)=>{const select=$('.daily-line-project',row);select.value=nextProject;select.onchange()});return true};
      houseInput.oninput=syncHouse;houseInput.onchange=acceptHeader;projectInput.onchange=acceptHeader;
      $('.daily-quick-project-open',group).onclick=()=>openQuickProject(group);
      $('.daily-house-add',group).onclick=()=>{if(!acceptHeader())return;const row=appendLine(group,emptyLine(groupProject(group),groupHouse(group)));calc();$('.daily-line-item',row).focus()};
      $('.daily-house-batch',group).onclick=()=>{if(acceptHeader())openBatch(group)};
      $('.daily-house-remove',group).onclick=()=>{group.remove();if(!body.children.length)createGroup('','',[emptyLine()]);calc()};
      return group;
    };
    const quickProjectCustomer=(group)=>{
      const current=state.projects.find((project)=>String(project.id)===String(groupProject(group)));
      if(current?.customer)return String(current.customer);
      const customerIds=new Set($$('.daily-house-project',body).map((select)=>state.projects.find((project)=>String(project.id)===String(select.value))?.customer).filter(Boolean).map(String));
      return customerIds.size===1?[...customerIds][0]:'';
    };
    const refreshProjectDropdowns=()=>{
      $$('select.daily-house-project,select.daily-line-project',body).forEach((select)=>{const selected=select.value;select.innerHTML=projectOptions(selected);select.value=selected});
    };
    const selectProjectForGroup=(group,project)=>{
      refreshProjectDropdowns();
      if(!group||!body.contains(group))return false;
      const select=$('.daily-house-project',group);select.value=String(project.id);return select.onchange?.()!==false;
    };
    ...
    quickProjectForm.onsubmit=async(event)=>{
      ...
      const existing=state.projects.find((project)=>String(project.customer)===String(customer)&&normalizedProjectName(project.name)===normalizedProjectName(name));
    ...
    lines.forEach((line,index)=>{const key=groupKey(line.project,line.house);if(!initialGroups.has(key))initialGroups.set(key,[]);initialGroups.get(key).push({...line,draftOrder:index})});
    draftSequence=lines.length;
    initialGroups.forEach((groupLines)=>createGroup(groupLines[0].project,groupLines[0].house,groupLines));
    $('#addDailyLine',layer).onclick=()=>{const group=createGroup('','',[emptyLine()]);calc();$('.daily-house-project',group).focus()};
    ...
    form.onsubmit=(event)=>{
      // Validate the draft before handing it to the unchanged store save path.
      const rows=$$('.daily-line',form),hasEmployee=$$('input[name="dailyEmployees"]:checked',form).length;
      const validRows=rows.filter((row)=>$('.daily-line-project',row).value&&(row.dataset.itemName||$('.daily-line-item',row).value.trim())&&number($('.daily-line-qty',row).value)>0);
      const invalidQuote=validRows.some((row)=>(row.dataset.sourceType==='quotation'||row.dataset.quotationId||row.dataset.quotationLineId)&&!quoteChoices($('.daily-line-project',row).value).some((item)=>String(item.quotationId)===row.dataset.quotationId&&String(item.quotationLineId)===row.dataset.quotationLineId));
      const seen=new Set(),duplicate=$$('.daily-house-group',body).some((group)=>{const project=groupProject(group),key=groupKey(project,groupHouse(group));if(!project)return false;if(seen.has(key))return true;seen.add(key);return false});
      if(!hasEmployee||!validRows.length||invalidQuote||duplicate){event.preventDefault();window.KushePhase1?.toast(duplicate?'此案場戶別已存在，請調整後再儲存':invalidQuote?'選取的報價項目已失效，請重新選擇已確認報價項目':'請至少選擇一位員工並填寫一筆施工項目');return}
      if($('button[type="submit"]',form).disabled){event.preventDefault();return}
      return submitDaily(event);
    };
    ...
    $('#confirmDailyHouseBatch',batchLayer).onclick=()=>{
      if(!batchTemplate||!body.contains(batchTemplate))return closeBatch();
      const houses=parseHouseBatch(batchInput.value);if(!houses.length){window.KushePhase1?.toast('請至少輸入一個戶別');batchInput.focus();return}
      const project=groupProject(batchTemplate),drafts=groupRows(batchTemplate).map((row)=>rowDraft(row)),skipped=[],created=[];
      houses.forEach((house)=>{if(findGroup(project,house)){skipped.push(house);return}createGroup(project,house,drafts.map((draft)=>cloneDailyLineDraft(draft,house)));created.push(house)});
      closeBatch();calc();window.KushePhase1?.toast([created.length?'已建立 '+created.join('、'):'',skipped.length?skipped.join('、')+' 已存在，已略過':''].filter(Boolean).join('；'));
    };
    batchLayer.onkeydown=(event)=>{if(event.key==='Escape'){event.preventDefault();closeBatch()}};
    calc();
  }
  async function submitDaily(event){event.preventDefault();const form=event.currentTarget,lines=$$('.daily-line',form).sort((a,b)=>number(a.dataset.draftOrder)-number(b.dataset.draftOrder)).map((row)=>({project:$('.daily-line-project',row).value,house:$('.daily-line-house',row).value.trim(),item:row.dataset.itemName||$('.daily-line-item',row).value.trim(),itemName:row.dataset.itemName||$('.daily-line-item',row).value.trim(),unit:$('.daily-line-unit',row).value.trim()||'式',qty:number($('.daily-line-qty',row).value),inputPrice:number($('.daily-line-price',row).value),unitPrice:number($('.daily-line-price',row).value),taxMode:$('.daily-line-tax',row).value,billable:row.dataset.pricingType!=='lump_sum'&&$('.daily-billable input',row).checked,sourceType:row.dataset.sourceType||'manual',pricingType:row.dataset.pricingType||'actual',quotationId:row.dataset.quotationId||'',quotationLineId:row.dataset.quotationLineId||'',quotationNo:row.dataset.quotationNo||'',quoteId:row.dataset.quotationId||'',quoteLineId:row.dataset.quotationLineId||'',lumpSumAmount:number(row.dataset.lumpSumAmount),workItemId:row.dataset.workItemId||''})),values={date:form.elements.date.value,employeeIds:$$('input[name="dailyEmployees"]:checked',form).map((input)=>input.value),lines,commissionEnabled:form.elements.commissionEnabled.checked,workMode:form.elements.workMode.value,workQty:number(form.elements.workQty.value),workRate:number(form.elements.workRate.value),note:form.elements.note.value.trim()},button=$('button[type="submit"]',form);button.disabled=true;button.textContent='儲存中…';try{await store.saveDailyBatch(values,editingDailyBatch);closeDrawer();render();window.KushePhase1?.toast('每日施工、抽成、點工與待請款已同步儲存')}catch(error){button.disabled=false;button.textContent='儲存每日施工';window.KushePhase1?.toast(`儲存失敗：${error.message}`)}}
  ...
  window.addEventListener('kushe:data-updated', () => { if (active && !quickProjectSaveActive) render(); });
