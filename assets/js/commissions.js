(function () {
  ...
  function openDailyDrawer(batchId='') {
    ...
    projectGroups.forEach((log)=>{(log.items||[]).forEach((item)=>lines.push({
      project:log.project,house:item.house||'',item:item.itemName||item.item||'',unit:item.unit||'式',qty:number(item.qty)||1,inputPrice:number(item.unitPrice??item.inputPrice??item.price),taxMode:item.taxMode||'未稅',billable:item.billable!==false,workItemId:item.workItemId||'',sourceType:item.sourceType||(item.quotationId?'quotation':'manual'),quotationId:item.quotationId||item.quoteId||'',quotationLineId:item.quotationLineId||item.quoteLineId||'',quotationNo:item.quotationNo||'',pricingType:item.pricingType||'actual',lumpSumAmount:number(item.lumpSumAmount)
    }))});
    ...
    const appendClone=(template,house='',after=template)=>{after.insertAdjacentHTML('afterend',lineHtml(cloneDailyLineDraft(rowDraft(template),house)));const clone=after.nextElementSibling;bindRow(clone);return clone};
    const closeBatch=()=>{batchLayer.hidden=true;batchTemplate=null;batchInput.value=''};
    const openBatch=(row)=>{batchTemplate=row;batchInput.value='';batchLayer.hidden=false;requestAnimationFrame(()=>batchInput.focus())};
    const bindRow=(row)=>{const project=$('.daily-line-project',row),item=$('.daily-line-item',row);project.onchange=()=>{item.value='';clearQuote(row);$('.daily-line-unit',row).value='式';$('.daily-line-price',row).value='0';$('.daily-line-tax',row).value='未稅';$('.daily-billable input',row).checked=true;refreshChoices(row);calc()};item.oninput=()=>{applyChoice(row);calc()};item.onchange=()=>{applyChoice(row);calc()};$$('input,select',row).forEach((input)=>{if(input!==project&&input!==item){input.oninput=calc;input.onchange=calc}});$('.daily-line-clone',row).onclick=()=>{const clone=appendClone(row);calc();$('.daily-line-house',clone).focus()};$('.daily-line-batch',row).onclick=()=>openBatch(row);$('.daily-line-remove',row).onclick=()=>{if(body.children.length>1)row.remove();else{item.value='';$('.daily-line-house',row).value='';clearQuote(row);$('.daily-line-unit',row).value='式';$('.daily-line-price',row).value='0'}calc()};if(row.dataset.sourceType==='quotation')applyChoice(row);else updateHint(row)};
    $('#addDailyLine',layer).onclick=()=>{body.insertAdjacentHTML('beforeend',lineHtml({project:'',house:'',item:'',unit:'式',qty:1,inputPrice:0,taxMode:'未稅',billable:true,workItemId:'',sourceType:'manual',pricingType:'actual'}));bindRow(body.lastElementChild);calc()};
    $$('.daily-line',body).forEach(bindRow);$$('input[name="dailyEmployees"],select[name="workMode"],input[name="workQty"],input[name="workRate"],input[name="commissionEnabled"]',form).forEach((input)=>{input.oninput=calc;input.onchange=calc});form.onsubmit=submitDaily;$$('.commission-drawer-backdrop,.commission-drawer-close,[data-cancel]',layer).forEach((button)=>button.addEventListener('click',closeDrawer));$$('[data-daily-house-batch-cancel]',batchLayer).forEach((button)=>button.onclick=closeBatch);$('#confirmDailyHouseBatch',batchLayer).onclick=()=>{if(!batchTemplate||!body.contains(batchTemplate))return closeBatch();const houseInput=$('.daily-line-house',batchTemplate),current=houseInput.value.trim(),plan=houseBatchPlan(current,batchInput.value);if(!plan.houses.length){window.KushePhase1?.toast('請至少輸入一個戶別');batchInput.focus();return}houseInput.value=plan.templateHouse;if(current&&!plan.cloneHouses.length){closeBatch();window.KushePhase1?.toast('戶別已存在，未新增重複列');return}let after=batchTemplate;plan.cloneHouses.forEach((house)=>{after=appendClone(batchTemplate,house,after)});closeBatch();calc();window.KushePhase1?.toast(`已建立 ${plan.houses.join('、')}`)};batchLayer.onkeydown=(event)=>{if(event.key==='Escape'){event.preventDefault();closeBatch()}};calc();
  }
  async function submitDaily(event){event.preventDefault();const form=event.currentTarget,lines=$$('.daily-line',form).map((row)=>({project:$('.daily-line-project',row).value,house:$('.daily-line-house',row).value.trim(),item:row.dataset.itemName||$('.daily-line-item',row).value.trim(),itemName:row.dataset.itemName||$('.daily-line-item',row).value.trim(),unit:$('.daily-line-unit',row).value.trim()||'式',qty:number($('.daily-line-qty',row).value),inputPrice:number($('.daily-line-price',row).value),unitPrice:number($('.daily-line-price',row).value),taxMode:$('.daily-line-tax',row).value,billable:row.dataset.pricingType!=='lump_sum'&&$('.daily-billable input',row).checked,sourceType:row.dataset.sourceType||'manual',pricingType:row.dataset.pricingType||'actual',quotationId:row.dataset.quotationId||'',quotationLineId:row.dataset.quotationLineId||'',quoteId:row.dataset.quotationId||'',quoteLineId:row.dataset.quotationLineId||'',lumpSumAmount:number(row.dataset.lumpSumAmount),workItemId:row.dataset.workItemId||''})),values={date:form.elements.date.value,employeeIds:$$('input[name="dailyEmployees"]:checked',form).map((input)=>input.value),lines,commissionEnabled:form.elements.commissionEnabled.checked,workMode:form.elements.workMode.value,workQty:number(form.elements.workQty.value),workRate:number(form.elements.workRate.value),note:form.elements.note.value.trim()},button=$('button[type="submit"]',form);button.disabled=true;button.textContent='儲存中…';try{await store.saveDailyBatch(values,editingDailyBatch);closeDrawer();render();window.KushePhase1?.toast('每日施工、抽成、點工與待請款已同步儲存')}catch(error){button.disabled=false;button.textContent='儲存每日施工';window.KushePhase1?.toast(`儲存失敗：${error.message}`)}}
