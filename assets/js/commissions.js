(function () {
  ...
  function options(rows, selected, emptyLabel, labelKey = 'name') {
    return `<option value="">${emptyLabel}</option>${rows.map((row) => `<option value="${esc(row.id)}" ${row.id === selected ? 'selected' : ''}>${esc(row[labelKey] || row.number || '—')}</option>`).join('')}`;
  }
    ...
    $('#commissionsApp').innerHTML = `
      ...
      <section class="commission-panel commission-table-panel">
        ...
        <div class="commission-table-wrap"><table class="commission-table">
          ...
          <tbody>${rows.map((row) => { const linkedPayroll = state.payroll.filter((item) => item.month === String(row.date || '').slice(0,7) && item.employee === row.employee); return `<tr data-row-id="${esc(row.id)}" data-payroll-records="${linkedPayroll.length}" data-payroll-commission="${linkedPayroll.filter((item) => item.status !== '已付款').reduce((sum,item) => sum + number(item.commission),0)}"><td>${esc(row.date || '—')}</td><td><b>${esc(label(state, 'employees', row.employee, row.employeeName || '—'))}</b></td><td>${esc(label(state, 'projects', row.project, row.projectName || '—'))}</td><td><span class="commission-source">${esc(row.sourceNo || (row.sourceType === 'daily-log' ? '每日業績' : '手動登錄'))}</span></td><td class="num">${money(grossOf(state, row))}</td><td class="num">${money(row.untaxedAmount)}</td><td class="num">${number(row.rate)}%</td><td class="num"><b>${money(row.commission)}</b></td><td><span class="commission-status ${row.status === '已列入薪資' ? 'is-settled' : 'is-unsettled'}">${row.status === '已列入薪資' ? '已結算' : '未結算'}</span></td><td><div class="commission-row-actions"><button type="button" data-edit="${esc(row.id)}">編輯</button><button type="button" data-delete="${esc(row.id)}">刪除</button></div></td></tr>`; }).join('') || '<tr><td class="commission-empty" colspan="10">此篩選條件下沒有業績紀錄。</td></tr>'}</tbody>
        </table></div>
      </section>
  ...
  function openDailyDrawerLegacy(batchId='') {
    ...
    const projectOptions=(selected='')=>`<option value="">請選擇案場</option>${state.projects.map((project)=>{const customer=label(state,'customers',project.customer,project.customerName||'未指定客戶');return `<option value="${esc(project.id)}" ${project.id===selected?'selected':''}>${esc(customer)}｜${esc(project.name)}</option>`}).join('')}`;
    const employeeChoices=state.employees.map((employee)=>`<label class="daily-employee-choice"><input type="checkbox" name="dailyEmployees" value="${esc(employee.id)}" ${employeeIds.has(employee.id)?'checked':''}><span><b>${esc(employee.name)}</b><small>抽成 ${number(employee.commissionRate)}%</small></span></label>`).join('');
  ...
  function openDailyDrawer(batchId='') {
    ...
    const workLog=logs.find((log)=>log.workMode&&log.workMode!=='none')||{},commissionEnabled=!logs.length||logs.some((log)=>number(log.performance)>0),layer=$('#commissionDrawerLayer');
    const projectOptions=(selected='')=>`<option value="">請選擇案場</option>${state.projects.map((project)=>{const customer=label(state,'customers',project.customer,project.customerName||'未指定客戶');return `<option value="${esc(project.id)}" ${String(project.id)===String(selected)?'selected':''}>${esc(customer)}｜${esc(project.name)}</option>`}).join('')}`;
    const employeeChoices=state.employees.map((employee)=>`<label class="daily-employee-choice"><input type="checkbox" name="dailyEmployees" value="${esc(employee.id)}" ${employeeIds.has(employee.id)?'checked':''}><span><b>${esc(employee.name)}</b><small>抽成 ${number(employee.commissionRate)}%</small></span></label>`).join('');
    const quoteChoices=(projectId)=>{const project=state.projects.find((row)=>String(row.id)===String(projectId));return project?(store.confirmedQuotationItems?.(project.id,project.customer)||[]):[]};
    const quoteLabel=(item)=>item.pricingType==='lump_sum'?`${item.item}｜總價 ${money(item.lumpSumAmount)}｜${item.quotationNo||'已確認報價'}`:`${item.item}｜${money(item.price)}/${item.unit||'式'}｜${item.quotationNo||'已確認報價'}`;
    const lineHtml=(line)=>{
      const listId=`dailyQuoteItems${++dailyLineSequence}`,choices=quoteChoices(line.project),selected=choices.find((item)=>String(item.quotationId)===String(line.quotationId)&&String(item.quotationLineId)===String(line.quotationLineId));
    ...
    const choicesForRow=(row)=>quoteChoices($('.daily-line-project',row).value);
    const clearQuote=(row)=>{row.dataset.sourceType='manual';row.dataset.quotationId='';row.dataset.quotationLineId='';row.dataset.pricingType='actual';row.dataset.lumpSumAmount='0';row.dataset.itemName=$('.daily-line-item',row).value.trim();$('.daily-line-unit',row).readOnly=false;$('.daily-line-price',row).readOnly=false;$('.daily-line-tax',row).disabled=false;$('.daily-billable input',row).disabled=false};
    const updateHint=(row)=>{const choices=choicesForRow(row),hint=$('.daily-quote-hint',row);if(!$('.daily-line-project',row).value){hint.textContent='請先選擇客戶／案場。';return}if(!choices.length){hint.textContent='此案場尚無已確認報價，可手動輸入施工項目。';return}if(row.dataset.sourceType==='quotation'){const type=row.dataset.pricingType==='lump_sum'?'總價（施工數量不增加待請款）':'實做實算';hint.textContent=`已連結 ${row.dataset.quotationNo||'已確認報價'}｜${type}`;return}hint.textContent='請搜尋並選擇已確認報價項目；亦可手動輸入。'};
    ...
    const applyChoice=(row)=>{const input=$('.daily-line-item',row),value=input.value.trim(),choices=choicesForRow(row);let matched=choices.find((item)=>quoteLabel(item)===value);if(!matched){const exact=choices.filter((item)=>item.item===value);if(exact.length===1)matched=exact[0]}if(!matched){clearQuote(row);updateHint(row);return false}row.dataset.sourceType='quotation';row.dataset.quotationId=matched.quotationId;row.dataset.quotationLineId=matched.quotationLineId;row.dataset.quotationNo=matched.quotationNo||'';row.dataset.pricingType=matched.pricingType;row.dataset.lumpSumAmount=String(number(matched.lumpSumAmount));row.dataset.itemName=matched.item;input.value=quoteLabel(matched);const unit=$('.daily-line-unit',row),price=$('.daily-line-price',row),tax=$('.daily-line-tax',row),billable=$('.daily-billable input',row);unit.value=matched.unit||'式';unit.readOnly=true;price.value=matched.pricingType==='lump_sum'?number(matched.lumpSumAmount):number(matched.price);price.readOnly=true;tax.value=matched.taxMode||'未稅';tax.disabled=true;billable.checked=matched.pricingType!=='lump_sum';billable.disabled=matched.pricingType==='lump_sum';updateHint(row);return true};
  ...
  async function remove(id) {
    ...
    await store.deleteCommission(id); render(); window.KushePhase1?.toast('業績已刪除，薪資連動已重算');
  }
