(function () {
  ...
  async function submitDaily(event){event.preventDefault();const form=event.currentTarget,lines=$$('.daily-line',form).map((row)=>({project:$('.daily-line-project',row).value,item:$('.daily-line-item',row).value.trim(),unit:$('.daily-line-unit',row).value.trim()||'式',qty:number($('.daily-line-qty',row).value),inputPrice:number($('.daily-line-price',row).value),taxMode:$('.daily-line-tax',row).value,billable:$('.daily-billable input',row).checked,workItemId:row.dataset.workItemId||''})),values={date:form.elements.date.value,employeeIds:$$('input[name="dailyEmployees"]:checked',form).map((input)=>input.value),lines,commissionEnabled:form.elements.commissionEnabled.checked,workMode:form.elements.workMode.value,workQty:number(form.elements.workQty.value),workRate:number(form.elements.workRate.value),note:form.elements.note.value.trim()},button=$('button[type="submit"]',form);button.disabled=true;button.textContent='儲存中…';try{await store.saveDailyBatch(values,editingDailyBatch);closeDrawer();render();window.KushePhase1?.toast('每日施工、抽成、點工與待請款已同步儲存')}catch(error){button.disabled=false;button.textContent='儲存每日施工';window.KushePhase1?.toast(`儲存失敗：${error.message}`)}}
